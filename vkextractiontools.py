...
parser.add_argument('--password', '-p', help='Пароль учётной записи VK, из под которой будут производиться запросы к API VK')
...
try:
    ...
    if error.code == 113:
        print('Аргументом опции --id является невалидный профиль Вконтакте')
    raise SystemExit
